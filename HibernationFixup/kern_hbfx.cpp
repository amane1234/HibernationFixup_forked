...
{
    // Call the original IOHibernateSystemSleep function and capture the result
    IOReturn result = FunctionCast(IOHibernateSystemSleep, callbackHBFX->orgIOHibernateSystemSleep)();

#ifdef DEBUG
    // Log a timestamp and the result when in DEBUG mode
    struct timeval tv;
    struct tm tm;
    microtime(&tv);
    gmtime_r(tv.tv_sec, &tm);
    DBGLOG("HBFX", "%02d.%02d.%04d %02d:%02d:%02d: IOHibernateSystemSleep is called, result is: 0x%x",
           tm.tm_mday, tm.tm_mon, tm.tm_year, tm.tm_hour, tm.tm_min, tm.tm_sec, result);
#endif

    // Get the current hibernate state from IOPMRootDomain
    ...
    // Check if the system is hibernating or returning from hibernation
    if (result == KERN_SUCCESS || ioHibernateState == kIOHibernateStateHibernating)
    {
        // Check the RTC extended memory and initialize NVStorage
        if (!callbackHBFX->checkRTCExtendedMemory())
        {
            callbackHBFX->initializeNVStorage();

            // Write IOHibernateRTCVariablesKey to NVRAM if it exists
            ...
            // Write IOHibernateSMCVariablesKey to NVRAM if it exists
        ...
        // Dump NVRAM variables if configured
        if (ADDPR(hbfx_config).dumpNvram)
        {
            uint32_t size;
            
            // Dump kGlobalBoot0082Key to kBoot0082Key
            ...
            // Dump kGlobalBootNextKey to kBootNextKey
            ...
            // Save the modified NVRAM to a file
...
IOReturn HBFX::IOHibernateSystemWake(void)
{
    ...
    // Call the original IOHibernateSystemWake function and capture the result
    ...
    // If there's a checkCapacityTimer, cancel it and set a new timeout
    ...
    // Handle specific wake types
    if (wakeType)
    {
        if (wakeType->isEqualTo(kIOPMrootDomainWakeTypeLowBattery) ||
...
void HBFX::IOPMrootDomain_evaluatePolicy(IOPMrootDomain* that, int stimulus, uint32_t arg)
{
    ...
    // Check if the autoHibernateMode configuration has DisableStimulusDarkWakeActivityTickle flag set
    ...
    {
        // If the stimulus is kStimulusDarkWakeActivityTickle, prevent it
    ...
    // Call the original IOPMrootDomain_evaluatePolicy function
           ...
           (params->sleepType == kIOPMSleepTypeDeepIdle || params->sleepType == kIOPMSleepTypeStandby || params->sleepType == kIOPMSleepTypeNormalSleep))
    {
        ...
        if (power_source && power_source->batteryInstalled()) {
            ...
            if (whenExternalPowerIsDisconnected && power_source->externalConnected()) {
                DBGLOG("HBFX", "Auto hibernate: external is connected, do not force hibernation");
            ...
            if (whenBatteryIsNotCharging && power_source->isCharging()) {
                DBGLOG("HBFX", "Auto hibernate: battery is charging, do not force hibernation");
                if (callbackHBFX->sleepPhase > kIOPMSleepPhase0)
                {
                    callbackHBFX->sleepServiceWake = false;
                    if (pmset_default_mode && standby_delay != 0 && !callbackHBFX->wakeCalendarSet && callbackHBFX->sleepPhase > kIOPMSleepPhase0)
            ...
            if (!power_source->isCharging())
            {
                ...
                if (whenBatteryIsAtWarnLevel && power_source->atWarnLevel()) {
                    DBGLOG("HBFX", "Auto hibernate: Battery is at warning level, capacity remaining: %d, force hibernation", power_source->capacityPercentRemaining());
                    forceHibernate = true;
                }

                if (whenBatteryAtCriticalLevel && power_source->atCriticalLevel()) {
                    DBGLOG("HBFX", "Auto hibernate: battery is at critical level, capacity remaining: %d, force hibernation", power_source->capacityPercentRemaining());
                    forceHibernate = true;
                }

                if (!forceHibernate && minimalRemainingCapacity != 0 && power_source->capacityPercentRemaining() <= minimalRemainingCapacity)
                {
                    DBGLOG("HBFX", "Auto hibernate: capacity remaining: %d less than minimal: %d, force hibernation", power_source->capacityPercentRemaining(), minimalRemainingCapacity);
                    forceHibernate = true;
                }
            }
        }
        ...
        if (!forceHibernate && whenLidIsClosed && lidIsOpen) {
            DBGLOG("HBFX", "Auto hibernate: clamshell is open, do not force hibernation");
            if (callbackHBFX->sleepPhase > kIOPMSleepPhase0)
            {
                callbackHBFX->sleepServiceWake = false;
                if (pmset_default_mode && standby_delay != 0 && !callbackHBFX->wakeCalendarSet && callbackHBFX->sleepPhase > kIOPMSleepPhase0)
        ...
        if (setupHibernate && doNotOverrideWakeUpTime && !forceHibernate)
        {
            ...
            {
                DBGLOG("HBFX", "%02d.%02d.%04d %02d:%02d:%02d: Auto hibernate: %d seconds to standby, cancel hibernation",
                       tm.tm_mday, tm.tm_mon, tm.tm_year, tm.tm_hour, tm.tm_min, tm.tm_sec, vars->standbyTimer);
                return result;
            }
            else
            {
                DBGLOG("HBFX", "%02d.%02d.%04d %02d:%02d:%02d: Auto hibernate: %d seconds to standby, enable hibernation",
                       tm.tm_mday, tm.tm_mon, tm.tm_year, tm.tm_hour, tm.tm_min, tm.tm_sec, vars->standbyTimer);
            }
        }
        ...
        if ((callbackHBFX->sleepPhase < kIOPMSleepPhase2) && setupHibernate)
        {
            ...
            DBGLOG("HBFX", "%02d.%02d.%04d %02d:%02d:%02d: Auto hibernate: sleep phase %d, set hibernation values",
        ...
        else if (callbackHBFX->sleepPhase == kIOPMSleepPhase2)
        {
            ...
            else
            {
                ...
                DBGLOG("HBFX", "%02d.%02d.%04d %02d:%02d:%02d: Auto hibernate: sleep phase %d, postpone hibernation",
                       tm.tm_mday, tm.tm_mon, tm.tm_year, tm.tm_hour, tm.tm_min, tm.tm_sec, callbackHBFX->sleepPhase);
            }
        ...
        {
            DBGLOG("HBFX", "%02d.%02d.%04d %02d:%02d:%02d: Auto hibernate: force hibernation...", tm.tm_mday, tm.tm_mon, tm.tm_year, tm.tm_hour, tm.tm_min, tm.tm_sec);
        }
        break;
    }
...
int HBFX::packA(char *inbuf, uint32_t length, uint32_t buflen)
{
    ...
    // Call the original packA function if available
    ...
    if (callbackHBFX->ml_at_interrupt_context && !callbackHBFX->ml_at_interrupt_context())
    {
        ...
        // Ensure interrupts are enabled for proper operation
        ...
        if (callbackHBFX->ml_get_interrupts_enabled())
        {
            ...
            // Enable preemption if it's not already enabled
            ...
            if (callbackHBFX->preemption_enabled())
            {
                ...
                // Write the panic information to NVStorage in parts
                while (pi_size > 0)
                {
                ...
                // Save the NVStorage data to a file
                callbackHBFX->nvstorage.save(FILE_NVRAM_NAME);
                callbackHBFX->sync(kernproc, nullptr, nullptr);
            }

            // Restore preemption state if it was disabled
            if (!preemption_enabled)
                callbackHBFX->disable_preemption();

            // Restore interrupts state if they were disabled
            if (!interrupts_enabled)
    ...
    else
    {
        ...
        if (rt) {
            ...
            if (buf) {
                ...
                SYSLOG("HBFX", "failed to create a temporary buffer");
            rt->put();
        }
        else
